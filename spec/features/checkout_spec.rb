...
feature 'Checkout', js: true do
  ...
  given!(:shipping_method) do
    ...
    calculator = Colibri::Calculator::Shipping::PerItem.create!(calculable: shipping_method, preferred_amount: 10)
  ...
  background do
    ...
    ActionMailer::Base.default_url_options[:host] = 'http://example.com'
    Colibri::Config[:enable_mail_delivery] = true

    # Bypass gateway error on checkout | ..or stub a gateway
    Colibri::Config[:allow_checkout_on_gateway_error] = true
  ...
  context 'without payment being required' do
    background do
      ...
      Colibri::Order.any_instance.stub has_available_payment: true
    ...
    scenario 'allow a visitor to checkout as guest, without registration' do
      ...
      within('#guest_checkout') { fill_in 'Email', with: 'colibri@test.com' }
    ...
    scenario 'associate an uncompleted guest order with user after logging in' do
      ...
      expect(page).to have_text 'Your order has been processed successfully'
      expect(Colibri::Order.first.user).to eq user
    end
    ...
    scenario 'associate an incomplete guest order with user after successful password reset' do
      ...
      fill_in 'colibri_user_email', with: 'email@person.com'
      ...
      token_url_regex = /^http:\/\/example.com\/user\/colibri_user\/password\/edit\?reset_password_token=(.*)$/
      token = token_url_regex.match(reset_password_email.body.to_s)[1]

      visit colibri.edit_colibri_user_password_path(reset_password_token: token)
    ...
    scenario 'allow a user to register during checkout' do
      ...
      fill_in 'Password', with: 'colibri123'
      fill_in 'Password Confirmation', with: 'colibri123'
      click_button 'Create'
      ...
      expect(page).to have_text 'Your order has been processed successfully'
      expect(Colibri::Order.first.user).to eq Colibri::User.find_by_email('email@person.com')
    end
  end
end
